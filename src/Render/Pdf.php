...
class Pdf
{
    ...
    public function __construct(PrintableContract $printable, ConfigContract $config = null)
    {
        ...
        $this->tcpdf->SetMargins($this->padding, $this->padding, $this->padding, true);
    ...
    public function render()
    {
        ...
        foreach ($tags as $tag) {
            $tagsCount++;
            $position = $tagsCount % $this->getPerPage();
            $position = $position == 0 ? $this->getPerPage() : $position;
            $this->tag($tag, $position);
            if ($this->getModel() == PrintableContract::MODEL_MULTIPLE && $position === $this->getPerPage()) {
                $this->tcpdf->addPage();
            }
        }
    ...
    private function barcode(PostalObject $tag, $x, $y)
    {
        ...
        $this->tcpdf->write1DBarcode($tag->getTagDv(), 'C128', $x + 5, $y + 5, 80, 18, null, $style);
     ...
     * @param string $margin
     */
    private function writeBold($h, $txt, $link = '', $fill = false, $align = '', $ln = false, $stretch = 0, $firstline = false, $firstblock = false, $maxh = 0, $wadj = 0, $margin = '')
    {
        $this->tcpdf->SetFont(null, 'B');
        $this->tcpdf->Write($h, $txt, $link, $fill, $align, $ln, $stretch, $firstline, $firstblock, $maxh, $wadj, $margin);
        $this->tcpdf->SetFont(null, null);
    }
}
